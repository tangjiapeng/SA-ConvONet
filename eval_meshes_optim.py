...
if not args.eval_input:
    out_file = os.path.join(generation_dir, 'eval_optim_meshes_full.pkl')
    out_file_class = os.path.join(generation_dir, 'eval_optim_meshes.csv')
else:
...
for it, data in enumerate(tqdm(test_loader)):
    ...
    if cfg['test']['eval_mesh']:
        mesh_file = os.path.join(mesh_dir, '%s_final.off' % modelname)

        if os.path.exists(mesh_file):
            try:
                mesh = trimesh.load(mesh_file, process=False)
                eval_dict_mesh = evaluator.eval_mesh_onlycd(
                    mesh, pointcloud_tgt, normals_tgt, remove_wall=cfg['test']['remove_wall'])
